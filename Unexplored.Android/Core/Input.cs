...
namespace Unexplored.Core
{
    public static class Input
    {
        ...
        public static GamePadState GamePadCurrenState { get; private set; }
        public static GamePadState GamePadLastState { get; private set; }
        ...
        private static bool firstLeftSide, secondLeftSide;
        private static bool firstRightSide, secondRightSide;
        ...
        private static void _PreventGameUpdate()
        {
            ...
            GamePadCurrenState = GamePad.GetState(0);
            var touch = TouchPanel.GetState();
            firstLeftSide = secondLeftSide = firstRightSide = secondRightSide = false;
            for (int i = 0; i < touch.Count; i++)
            {
                currentMousePosition = touch[i].Position;

                float partSize = Constants.WindowWidth / 4f;
                float mx = currentMousePosition.X;

                float s = 80 * Constants.WindowWidthScale;
                if (mx > 0 && mx < partSize - s)
                    firstLeftSide = true;
                if (mx > partSize - s && mx < partSize * 2 - s)
                    secondLeftSide = true;
                if (mx > partSize * 2 + s && mx < partSize * 3 + s)
                    firstRightSide = true;
                if (mx > partSize * 3 + s)
                    secondRightSide = true;
            }
            ...
            if (CurrentKeyboardIsDown(Keys.W) || CurrentKeyboardIsDown(Keys.Up) ||
                CheckJoystickAxeLessZero(1) || GamePadCurrenState.DPad.Up == ButtonState.Pressed)
            { vertical = +1; up = true; }
            // Down
            ...
            if (CurrentKeyboardIsDown(Keys.Space)
                || CheckJoystickButton(0) || GamePadCurrenState.Buttons.A == ButtonState.Pressed)
                jump = true;

            // Attack
            if (CurrentKeyboardIsDown(Keys.Z)
                || CheckJoystickButton(1) || GamePadCurrenState.Buttons.B == ButtonState.Pressed)
                attack = true;
            ...
            currentInputStatus[InputType.FirstLeft] = firstLeftSide;
            currentInputStatus[InputType.SecondLeft] = secondLeftSide;
            currentInputStatus[InputType.FirstRight] = firstRightSide;
            currentInputStatus[InputType.SecondRight] = secondRightSide;
